...
# def filter_participations(request):
    ...
    # if user_role == "Hod":
        # filtered_parents = [p for p in filtered_parents if p.get("stf_id") in stf_id]
    # else:
...
# def export_to_excel(parents, children, selected_options):
    ...
    # if v not in [None, '', []] and (is_scalar(v) and not pd.isna(v) or not is_scalar(v))
# }
...
def export_to_excel(parents, children, selected_options):
    ...
    for child in children:
        ...
        # Get media files information and create hyperlinks using MEDIA ID
        media_files = child.get('media_files', [])
        attachment = "No media files"
        
        if media_files:
            # Create hyperlinks for each media file using MEDIA ID URLs
            media_links = []
            for media in media_files:
                file_name = media.get('file_name', 'Unknown')
                # Use the direct API URL from media data (which uses media ID)
                media_url = media.get('direct_api_url', '')
                
                if media_url:
                    # Create Excel hyperlink formula with media ID URL
                    hyperlink_formula = f'=HYPERLINK("{media_url}", "{file_name}")'
                    media_links.append(hyperlink_formula)
                else:
                    # Fallback: try to construct URL using media ID if direct_url is not available
                    media_id = media.get('media_id')
                    if media_id:
                        fallback_url = f"{API_STUDIO_URL}crudapp/view/media/naac01_faculty_participation_dc2_media/{media_id}"
                        hyperlink_formula = f'=HYPERLINK("{fallback_url}", "{file_name}")'
                        media_links.append(hyperlink_formula)
                    else:
                        # Final fallback: use child ID as before
                        child_id = child.get('psk_id', '')
                        if child_id:
                            fallback_url = f"{API_STUDIO_URL}crudapp/view/media/naac01_faculty_participation_dc2_media/{child_id}"
                            hyperlink_formula = f'=HYPERLINK("{fallback_url}", "{file_name}")'
...
def export_to_pdf(parents, children, selected_options):
    """
    Export filtered data to PDF format with clickable media file links using MEDIA ID URLs
    """
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename="participations_filtered.pdf"'
    ...
    if children:
        ...
        for child in children:
            ...
            # Get media files information and create clickable links with MEDIA ID URLs
            media_files = child.get('media_files', [])
            media_links = []
            ...
            if media_files:
                for media in media_files:
                    ...
                    if media_url:
                        # Create a clickable link with the filename as the visible text using media ID URL
                        link_text = f'<a href="{media_url}" color="blue"><u>{file_name}</u></a>'
                        media_links.append(link_text)
                    else:
                        # Fallback: try to construct URL using media ID if direct_url is not available
                        ...
                        if media_id:
                            fallback_url = f"{API_STUDIO_URL}crudapp/view/media/naac01_faculty_participation_dc2_media/{media_id}"
                            link_text = f'<a href="{fallback_url}" color="blue"><u>{file_name}</u></a>'
                            media_links.append(link_text)
                        else:
                            # Final fallback: use child ID as before
                            child_id = child.get('psk_id', '')
                            if child_id:
                                ...
                                media_links.append(f"{file_name} (No URL)")
            ...
            child_data.append([
                ...
                Paragraph(str(institution), styles['Normal']),
                Paragraph(str(details), styles['Normal']),
                Paragraph(media_info, styles['Normal'])  # This will render as clickable links with media ID URLs
            ])
